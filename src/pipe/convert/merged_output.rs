...
#[derive(Debug)]
pub struct MergedOutput2<R1, R2> {
    pub stream0: R1,
    pub stream1: R2,
}

impl<R1, R2> MergedOutput2<R1, R2> {
    pub fn new(stream0: R1, stream1: R2) -> MergedOutput2<R1, R2> {
        MergedOutput2 { stream0, stream1 }
    }
}

impl<R1: AsyncRead + Unpin, R2: AsyncRead + Unpin> AsyncRead for MergedOutput2<R1, R2> {
    fn poll_read(
